...
import { resolveRustEngineCommand } from "../src/engine/rust-engine.js";
...
describe("engine scan data bridge", () => {
  ...
  it("fails closed when the configured Rust engine cannot execute", async () => {
    const originalEngineBin = process.env.DRIFT_ENGINE_BIN;
    const originalFallback = process.env.DRIFT_ALLOW_TYPESCRIPT_ENGINE_FALLBACK;
    process.env.DRIFT_ENGINE_BIN = "/tmp/drift-missing-engine-bin";
    delete process.env.DRIFT_ALLOW_TYPESCRIPT_ENGINE_FALLBACK;

    try {
      await expect(collectScanData(input)).rejects.toThrow(/DRIFT_ENGINE_BIN is invalid/);
    } finally {
      ...
      if (originalFallback === undefined) {
        ...
        process.env.DRIFT_ALLOW_TYPESCRIPT_ENGINE_FALLBACK = originalFallback;
      }
    }
  });
