...
use drift_engine::{
    ...
    Fact, FactKind, FindingStatus, ParsedDiff, Severity, classify_findings_against_baseline,
    RuleFinding, classify_findings_against_diff, materialize_direct_data_access_findings,
};
...
fn graph_direct_data_access_findings(
    ...
    for edge in resolved_import_edges {
        ...
        let import_source = string_metadata(import_node, "source").unwrap_or(import_node.label.as_str());
        ...
        let file_path = string_metadata(import_node, "file_path").unwrap_or_default().to_string();
        ...
        let evidence_id = edge
            ...
            .unwrap_or_else(|| format!("evidence_graph_{}", stable_hash(&format!("{}:{}", file_path, import_source))[..16].to_string()));
...
fn is_forbidden_import_source(import_source: &str, forbidden_imports: &[String]) -> bool {
    forbidden_imports.iter().any(|forbidden| import_source == forbidden || import_source.contains(forbidden))
}
