...
supported_sqlite_schema_version: 31,
  ...
  it("reports parser gap v2 summaries by capability and contract kind", async () => {
    const { databasePath, repoId } = await seedStartedDoctorState("drift-parser-gap-v2-status-");
    const storage = openDriftStorage({ databasePath });
    storage.migrate();
    const scanId = storage.listScanManifests(repoId)
      .find((scan) => scan.status === "completed" && !scan.id.startsWith("scan_baseline_") && !scan.id.startsWith("scan_check_"))!.id;
  ...
  it("starts onboarding with accept-defaults, materializes contract, and baselines existing violations", async () => {
    ...
    expect(result.exitCode).toBe(0);
    // BB-3: the acceptance line names what was decided. This fixture is the warn-shaped case -
    // every file in scope violates, so `baseline_coverage_direction` reads the convention as an
    // aspiration - and the old output said only "Accepted default convention.", identically to the
    // block-shaped case.
    expect(result.stdout).toContain('Accepted "api_route_no_direct_data_access" in WARN mode');
    expect(result.stdout).toContain("1 existing violation baselined");
  ...
  it("runs doctor before local state exists and prints a clean next command", async () => {
    ...
    expect(result.stdout).toContain("Runtime: Drift CLI 0.1.0, SQLite schema 31");
...
storage_schema_version: 31
  ...
  it("blocks check enforcement when the explicit TypeScript fallback scanner is used", async () => {
    ...
    try {
      ...
      const payload = JSON.parse(result.stdout);
      expect(payload.check).toMatchObject({
        // E-1 (S1-02): the fallback path exits 3 with blocked_reasons
        // ["typescript_fallback_used"] - a refusal, and the status now says so.
        status: "refused",
        fallback_status: {
          fallback_used: true,
