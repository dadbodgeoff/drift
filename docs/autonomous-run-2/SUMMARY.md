...
- **T123** GitHub Action (design-only) — Workflow authored as .github/workflows/drift-check.example.yml - named .example.yml so it does NOT run - plus docs/ci-integration.md stating exactly what is and is not verified. Nothing pushed. **PREMISE FALSE, corrected 2026-08-11:** the `.example` infix does not stop GitHub from running a workflow. The file declared `on: pull_request` and ran on PR #101, failing at an `npm ci` step that could never work in this pnpm workspace. "Design-only" was not a property of the artifact, only of the intent. Renamed to `drift-check-self.yml` and fixed; the honest version of T123 is that the self-check ran for the first time on 2026-08-11.
